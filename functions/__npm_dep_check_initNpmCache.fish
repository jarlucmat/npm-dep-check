function __npm_dep_check_initNpmCache
	set -f npmJson (npm ls --all --json)
	set -f returnCode $status
	if test $returnCode -ne 0
		begin
			printf "\nNPM exited with code %s\nCheck your project and run npm (clean-)install before using this tool!" $returnCode
		end >&2
		return 1
	end

	set -f allDeps (echo $npmJson |\
